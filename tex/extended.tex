...
\begin{abstract}
  ...
  \href{https://github.com/iblech/constructive-maximal-ideals/}{\textnormal{https://github.com/iblech/constructive-maximal-ideals/}\!.}
...
still surjective. Hence we have a contradiction to basic linear algebra over
fields.\hfill\qed
\end{quote}
...
theory~\textsc{zf}) the axiom of choice~\cite{scott:prime,hodges:krull,banaschewski:krull,erne:krull,howard-rubin:ac}.
...
clickable link to the corresponding machine-checked statement. In several cases,
the formalized version is slightly more general than the version in this text;
conversely, in cases where we have felt that no additional insight is gained,
the formalization only implements a special case.
...
\begin{quote}\small
  ...
  and are thereby nilpotent.\hfill\qed
\end{quote}
...
By elementary row and column operations, the matrix can be transformed to
the form~$\left(\begin{smallmatrix}1 & 0 \\ 0 &
M'\end{smallmatrix}\right)$, where the induced linear map of the submatrix~$M'$ is again
surjective. Hence~$1 = 0$ by the induction hypothesis (applied to~$A$).\hfill\qed
\end{quotation}
...
single-valued and surjective as expected, but also has the property that, up to double negation, it is
the case that every desired finite pattern occurs among its
values: In the forcing extension it holds that for every finite list~$\sigma =
x_0 \ldots x_{k-1}$, it is \emph{not~not} the case that there is a number~$n \in
...
\begin{definition}Let~$X$ be a set. An \emph{inconsistency predicate} on~$X$
is a predicate~$\Incbare$ on~$P(X)$ which
\begin{enumerate}
...
by~$M$ contains~$0$, that is that some (perhaps empty) product~$m_1 \ldots m_n$ of elements
...
\begin{remark}\label{rem:inc-by-closure}The inconsistency predicates of
Examples~\ref{ex:inc-ring},~\ref{ex:inc-ring2},~\ref{ex:inc-module} and~\ref{ex:inc-formulas} are
all induced by \emph{algebraic closure operators} as reviewed
in~\cite[Example~1]{wessel-schuster:radical}; in these cases, $\Inc{U}$
is actually defined in terms of the closure of the subset~$U$.
\end{remark}
...
with an inconsistency predicate. Then every subset~$A_0$ of~$X$
has an extension~$A \supseteq A_0$ which is \emph{maximal equiconsistent} in
that for every subset~$B$ of~$X$:
\[ A \subseteq B \quad\wedge\quad \neg \Inc{B} \quad\Longrightarrow\quad A = B. \]
\end{theorem}

\begin{proof}
We verify that~$A$ as in Construction~\ref{constr:inc} is as desired.
Let~$x \in B$. Then~$\neg \Inc{(A \cup \{ x \})}$, as~$\Inc{(A \cup \{x\})}$
would imply~$\Inc{B}$ by monotonicity. Hence~$x \in A$ by
Lemma~\ref{lemma:omnibus-inc}(b).
\end{proof}

\begin{remark}If an inconsistency predicate is induced by an algebraic
closure operator (Remark~\ref{rem:inc-by-closure}), then every subset is
equiconsistent with its closure; whence the maximal equiconsistent extensions
provided by Theorem~\ref{thm:max-inc-countable} are automatically closed under
the given operator. For instance, maximal equiconsistent
extensions with respect to the inconsistency predicate of Example~\ref{ex:inc-ring}
are automatically ideals and extensions with respect to the predicate of
Example~\ref{ex:inc-formulas} are automatically deductively closed.
\end{remark}

Extending the empty subset by Theorem~\ref{thm:max-inc-countable} has a number of corollaries, such as:
\begin{enumerate}
...
multiplicatively saturated (closed under divisors) and a prime filter in the weak sense that~$x + y \in
...
that~$A$ is reduced, this ring of fractions is a field in the sense that nonzero
...
\end{enumerate}
By extending a given subset instead of the empty one, all these results also
admit relative versions. For instance, every submodule~$U$ of a finitely generated
module over a countable ring can be extended to a maximal submodule~$U'$ such
that~$U' = M \Rightarrow U = M$. Some corollaries of Theorem~\ref{thm:max-inc-countable}
are usually only stated in their relative form:
\begin{enumerate}
\item[(5)] Every set of sentences over a countable signature can be completed to a

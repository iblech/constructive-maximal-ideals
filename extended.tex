...
\begin{enumerate}
\item Every countable ring has a maximal ideal (Example~\ref{ex:inc-ring}).
\item Every finitely generated module
over a countable ring has a maximal proper submodule
(Example~\ref{ex:inc-module}).
\item Every set of sentences over a countable signature can be completed to a
...
\item Every countable vector space over a residue field has a maximal linearly
...
Fundamental to Construction~\ref{constr:inc} is its step-by-step nature, the
elements are inspected one-by-one. If an infinite sequence of elements does not
suffice to exhaust the set, it is conceivable to take a limit step and continue
sequentially from then on.

Let~$(I,{<})$ be a well-ordering, that is a set~$I$ equipped with a XXX
...
\begin{enumerate}
\item[(a)] The set~$A$ is consistent: If~$O(A)$, then
also~$O(\emptyset)$.

...
For suitable choice of the entailment relation~$({\vdash})$, we recover the
...
Example~\ref{ex:forcing-lindenbaum} is the special case where~$S$ is the set of
atomic propositions and where~$\mathcal{A} \vdash \mathcal{B}$
iff~$\bigwedge_{\varphi \in \mathcal{A}} \varphi \vdash \bigvee_{\psi \in
\mathcal{B}} \psi$. The forcing notion of Example~\ref{ex:notion-max} is the
special case where the entailment relation is that
of~\cite[p.~72]{schuster-wessel:krull}.
\end{example}
...
* cite: Hatzikiriakou, K. (1991). Minimal prime ideals and arithmetic comprehension .
  The Journal of Symbolic Logic, 56(01), 67–70. doi:10.2307/2274904 

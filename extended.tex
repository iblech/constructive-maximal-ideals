...
$\{ \sigma \tau \,|\, \tau \in L, a \in \sigma \tau
\}$. (By~$\sigma y$, we mean the enlarged list with~$r$ as an
added element at the end. By~$\sigma \tau$, we mean the concatenation
...
with~$\neg O(\sigma)$ for which the disjunction
\[ \sigma \preceq \tau \quad\text{or}\quad O(\sigma \vee \tau) \]
holds for all elements~$\tau \in P$. The correspondence is by mapping a
...
\[ \Cov(\sigma) \defeq \{ M \in P({\downarrow}\sigma) \,|\,
  ...
  \}. \]
% xxx: not quite correct(?), need atomless(?)
% Up to the double negation which disappears in a classical metatheory, sets~$M$ as displayed are called
% \emph{dense} in the literature on classical set-theoretic forcing, and
...
Conversely, every pointfree space gives rise to a forcing notion such that its
filters are in canonical one-to-one correspondence with the points of the space.
For instance, let a pointfree space be given by a frame~$L$ as in locale

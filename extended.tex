...
In the present text we study a variant of this construction, due to Berardi and
...
XXX The second topic of this paper is the general toolbox and dynamical algebra. Since its multiple
independent inventions by Dominique Duval and Paul
Lorenzen~\cite{XXX}, the general toolbox of the dynamical method has been widely
...
\section{Generalization to arbitrary inconsistency predicates on well-orders}

This section generalizes the construction of Section~\ref{sect:constr} from
countable rings to arbitrary countable sets equipped with an
\emph{inconsistency} predicate on their powerset. In a second step, we relax
the countability assumption to a more general well-foundedness requirement.
...
directed joins. For our purposes, the level of generality displayed in the
...
\begin{enumerate}
\item monotone: if~$U \subseteq V$ and~$O(U)$, then~$O(V)$, and
...
A particular example of a directed join is the union of an ascending sequence
of subsets: For an inconsistency predicate~$O$, if~$O(\bigcup_{n \in \NN}
U_n)$, then there is a number~$n$ such that~$O(U_n)$.

\begin{example}Let~$A$ be a ring. For any subset~$M \subseteq A$, let~$O(M)$
state that~$1 \in (M)$, where~$M$ is the ideal generated by~$M$. Then~$O$ is an
inconsistency predicate.\end{example}

\begin{example}Let~$M$ be a module over a ring. For any subset~$U \subseteq M$,
let~$O(U)$ denote that~$U = M$. Then~$O$ is an inconsistency predicate if and
only if~$M$ is finitely generated.\end{example}

\begin{example}Let~$X$ be the set of sentences over a fixed first-order
...
We then have the following generalization of the construction in
Section~\ref{sect:constr}.
...
O(A_n)$. The result then follows because~$O$ splits directed joins.
...
(2)$. For~$(2) \Rightarrow (3)$, suppose that~$x_n \in A$.
If~$O(A\cup\{x_n\})$, then~$O(A)$ because~$A\cup\{x_n\}=A$ and hence~$O(A_0)$
by part~(a).
\end{proof}

\begin{theorem}Let~$X$ be a countable set equipped with an inconsistency
predicate. Then every subset of~$X$ has a maximal equiconsistent
extension.\end{theorem}
...
%\begin{theorem}Let~$X$ be a countable set equipped with an inconsistency
%predicate~$O$. Let~$x_0,x_1,\ldots$ be an enumeration of~$X$.
%Let~$A_0 \subseteq X$ be an arbitrary subset. Recursively define
%\[ A_{n+1} \defeq A_n \cup \bigl\{ x_n \,|\, O(A_n \cup \{x_n\}) \Rightarrow O(A_0)
%\bigr\}. \]
%Then:
%\begin{enumerate}
%\item For every number~$n$, if~$O(A_n)$ then~$O(A_0)$.
%\item For every number~$n$, the following are equivalent:
%$
%\end{enumerate}
%\end{theorem}

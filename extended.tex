...
\begin{example}Assume that~$A$ is a residue field. Let~$f \in A[X]$ be a nonconstant monic
...
Let~$f_0,f_1,\ldots$ be an enumeration of all monic polynomials over~$A$;
successively use Kronecker or universal splitting algebra steps; and take the
direct limit to obtain a residue field~$K_0$. Over this field, all the
polynomials~$f_i$ split into linear factors. If~$g$ is a polynomial over~$K_0$,
...
monic polynomial~$f_i$ over~$A$. This polynomial already splits into linear
...
so classically~$K_0$ is the desired algebraic closure. Constructively, this can
...
closure---if~$g$ is a monic polynomial over~$K_\omega$, its coefficients will

...
commutative algebra. However, it requires an enumeration~$\NN
\twoheadrightarrow A$ of the ring elements. This section deals with the case
that no such enumeration exists or is available.

Without much difficulty, the same technique can be used in situations where the
...
renders~$V^\nabla$ a better (more well-adapted) universe to work in than the
...
toposes~\cite{xxx}. The objective of this section is to give a purely
syntactical account. We circumvent the actual construction of~$V^\nabla$ by
...
we are prepared to take: A covering system specifies for each \emph{forcing condition}~$\sigma$
...
\begin{quote}If~$\tau \leq \sigma$ and~$R \in \Cov(\sigma)$, there should be a
...
for~$i=0,\ldots,n-1$, and declare~$\tau \leq \sigma$ iff~$\sigma$ is a prefix
...
\begin{example}We can also directly force the existence of a maximal ideal,

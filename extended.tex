...
\begin{quote}\small
\textbf{Proposition.} Let~$M \in A^{n \times m}$ be a matrix with more rows
...
coherent sequent (over the signature of commutative rings), the completeness
theorem for coherent logic implies a priori that there exists a proof in
coherent logic, so in particular a constructive proof. The completeness
...
several techniques meet this challenge. Section~\ref{sect:krull} shows that
also the explicit maximal ideal construction can be used to this end---assuming
...
The aim is to extract a direct computational argument that~$1 = 0$ from such a
classical proof. To this end, the toolbox of dynamical algebra suggests a

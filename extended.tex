...
\begin{lemma}\label{lemma:via-generators}For for every number~$n \in \NN$, the following are equivalent:
\begin{enumerate}
...
splinters into several inequivalent notions~\cite{XXX}. Common to most of them
is that firstly they refer only to finitely generated ideals and that secondly
stabilization is weakened to \emph{stalling}: For instance, the
Richman--Seidenberg condition~\cite{XXX} is that every infinite ascending chain
...
instance trees~\cite{XXX}, processes~\cite[Section~3.9]{blechschmidt:phd} or
sheaves of sequences, but keep these two restrictions, for without them not
...
finite generation of the approximations as we cannot. Hence the classical
finiteness property is not reproduced in a strong sense.

For the exceptional \emph{anonymous} Noetherian condition, stating that every
...
and Lombardi's approach to Noetherian rings~\cite{coquand-lombardi:krull}, we

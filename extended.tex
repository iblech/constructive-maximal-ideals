...
\begin{abstract}
  ...
  XXX
  By connecting dynamical algebra with
...
%EXAMPLE
%
%well-founded, abstract inconsistency predicate


\subsubsection{Generalization to the uncountable case}
...
in order to explore the range of set-theoretic possibility. However, they work
in a classical setting without issues of constructivity in mind.

To apply the iterative step-by-step construction of a maximal ideal to a
...
computational interpretation---in which~$A$ appears countable; this extended
...
first-order logic. Hence we have the metatheorem that \emph{countability
assumptions from intuitionistic proofs of first-order statements can always be
mechanically eliminated:} If our aim is to verify some first-order
...
In Section~\ref{sect:wlog} we also turn to the general toolbox of dynamical
...
The aim is to extract a direct computational argument that~$1 = 0$ from such a classical
proof. To this end, the toolbox of dynamical algebra suggests a \emph{dynamical rereading} of
the given proof. Instead of appealing to the transfinite in order to concoct
a completed maximal ideal~$\mmm$, we make do with finitely generated
approximations to a maximal ideal, starting out with the zero ideal~$\aaa =
(0)$ and repeatedly refining this approximation over the course of the proof.

The fundamental property of a maximal ideal~$\mmm$ is that for every ring
...
approximation from~$\aaa$ to~$\aaa + (x)$, pretend that the first alternative
...
\item[(2)] Friedman's continuation trick as in Barr's theorem.
\end{enumerate}
\end{quote}
...
verbatim. The argument that~$\neg O(A_n)$ for every index~$n \in I$ is now by
transfinite induction and requires some adaptation.
...
inductive hypothesis); or~$O(X)$ together with the information

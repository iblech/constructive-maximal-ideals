...
complex numbers in intuitionistic mathematics.\footnote{Residue fields have
...
linearly independent, finitely generated vector spaces do (\emph{not not}) have
a finite basis, monic polynomials possess splitting fields and Noether
normalization is available. The rarer \emph{geometric fields} -- those kinds of
fields for which every element is either invertible or zero -- are required to
ensure that kernels of matrices are finite dimensional or that bilinear forms
are diagonalizable.}
...
\begin{quote}\small
  ...
  contained in the intersection of all prime ideals and thereby
  nilpotent.\medskip
...
general situations, while for the purposes of this specific challenge we could also
...
prime ideals. In contrast, the following test case crucially rests on the
maximality of the ideal~$\mmm$ constructed in Section~\ref{sect:constr}.
...
surjective. Hence~$1 = 0$ by induction.

By the residue field property, thus all matrix entries are zero. Hence~$1 = 0$.
\end{proof}

A more significant case study is Suslin's lemma, the fundamental and originally
unconstructive ingredient in his second solution of Serre's
problem~\cite{suslin:structure}. The classical proof, concisely recalled in
Yengui's constructive account~\cite{yengui:maximal}, proceeds by quotienting by
all maximal ideals. The construction of Section~\ref{sect:constr} offers a
constructive substitute. However, since greatest common divisor computations
are required in the quotients, it is not enough that the quotients are residue
...
Hence we have the metatheorem, due to Joyal and Tierney, that \emph{countability assumptions from
...
Löwenheim--Skolem theorem of model theory.} Crucially, the first-order restriction is only on the form
...
\begin{remark}The dynamic approach as presented in the textbook by Lombardi and
Quitté~\cite[Section~XV.6]{lombardi-quitte:constructive-algebra}
...
This approach is intimately connected with the technique of this section, even
though it is cast in entirely different language.
We briefly indicate the relationship here and report
details in future work.

If a given classical proof appeals to the maximality condition ``$x \in \mmm$
or~$1 \in \mmm + (x)$'' (``$x$ is zero modulo~$\mmm$ or invertible
modulo~$\mmm$'') only for a finite number~$x_0,\ldots,x_{n-1}$ of ring elements
fixed beforehand, we can apply the construction in Section~\ref{sect:constr} to
this finite enumeration and use the resulting ideal~$\mmm_n$ as a partial
substitute for the elusive maximal ideal. The tools from pointfree topology
driving Joyal and Tierney's metatheorem widen the applicability of this partial
substitute to cases where the selection of which ring elements to inspect is
not fixed beforehand, by dynamically growing the partial enumeration as the
proof runs its course. If required, a continuation-passing style transform can
be applied to upgrade the partial and dynamic maximal ideal from one only
satisfying the weaker condition~``$1 \not\in \mmm + (x)$ implies~$x \in
\mmm$'' to one satisfying the stronger condition~``$x \in \mmm$ or~$1 \in \mmm
+ (x)$''.
\end{remark}
...
cite Lemma 2.1 of ishihara-khoussainov-nerode:decidable-kripke-models?

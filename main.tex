...
\mmm + (y)$. If~$1 \in \mmm + (y)$, then~$x \in (x) \cdot \mmm + (xy)
...
\begin{example}If~$A$ is a local ring, then~$\mmm = A \setminus A^\times$.\end{example}
...
p.2: The notation "1 = 0 in A" is standard in Martin-Loef Type Theory, but in
(Bishop-style) informal constructive mathematics is better to use 0 =_A
1.

p.3: For the elements of A you use symbols like x, y, a, u, v. A single,
uniform notation seems to me better.

p.3: In the proof of Corollary 1.5, second line: just write x in m + (xy)

p.3: In the proof of Corollary 1.6, second line, write: by Lemma 1.3

p.4 Example 1.9: explain the term A^x

p.5 Are there interesting examples of subcountable rings that are not
countable?

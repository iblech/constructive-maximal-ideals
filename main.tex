...
\begin{abstract}
  ...
  minimal logic, do without any kind of choice. By a functional
...
maximal ideal of~$A$ is to apply Zorn's lemma to the poset of proper ideals
...
detachable, then the ideal~$(M)$ generated by~$M$ can be constructed
...
\end{enumerate}
Assume furthermore that for any finitely generated ideal~$\aaa \subseteq
...
For claim~(3), let an element~$x_n \in A$ be given. By
...
(radical and double negation stable) prime ideal~$\ppp \subseteq A$ such that~$x \not\in \ppp$.
\end{corollary}
...
This issue is also noted, though not resolved, in related work on the
constructive content of Gödel's completeness
theorem~\cite[p.~11]{herbelin-ilik:henkin}.
The issue does not arise in the context of~\textsc{pa}, where the law of
excluded middle allows us keep the logical complexity bounded: We can blithely define
...
in~\cite{blechschmidt:generalized-spaces,johnstone:art,johnstone:point,vickers:continuity,vickers:locales-toposes,belanger-marquis:pointless,johnstone:baby-elephant}. A predicative account on the basis of~\cite{maietti:au,vickers:sketches,crosilla:predicativity}
is also possible.}
...
view of maximal ideals as convenient fictions. Maximal ideals can

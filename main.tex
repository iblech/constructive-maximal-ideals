...
stages~$\mmm_n$. For instance, already Krull in his 1929 Annals contribution~\cite[Hilfssatz]{krull:ohne} and Mines, Richman, Ruitenburg, Lombardi and Quitteé in their books on constructive
...
complex numbers.
Each of the latter in fact is a \emph{Heyting field}: that is,
a residue field which also is a \emph{local ring} in the sense that if a finite
sum of ring elements is invertible then one of the summands is.
...
in~\cite{blechschmidt:generalized-spaces,johnstone:point,vickers:continuity,vickers:locales-toposes,belanger-marquis:pointless}. A predicative account on the basis of~\cite{maietti:au,vickers:sketches,crosilla:predicativity}
...
FUTURE: compare Noetherian
XXX cite prime, maximal, ...
